...
export class DataService {
  ...
  readonly URL_PATTERN = "http://localhost:8080/guest/{tableId}/state";
  ...
  private sendRequest(tableId: string) {
    ...
    const url = this.URL_PATTERN.replace("{tableId}", tableId);
    const options = {withCredentials: true};
    const httpObservable = this.http.get(url, options).pipe(first());

    httpObservable.subscribe(data => this.dataObservable.next(data));
    httpObservable.subscribe(() => this.dataAvailableObservable.next(true));
  }
